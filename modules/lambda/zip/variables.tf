...
variable "source_dir" {
  type        = string
  description = "Lambda関数のソースディレクトリ"
}

variable "output_path" {
  type        = string
  description = "Lambda関数のZIPファイルの出力先パス"
}

variable "excludes" {
  type        = list(string)
  description = "ZIPに含めたくないファイルおよびフォルダ。例) [\".dockerignore\", \"Dockerfile\", \"README.md\", \"lambda_function.zip\"]"
  default     = []
}
...
variable "handler" {
  type        = string
  description = "Lambda関数のハンドラー(仮にmain.pyのhandler関数をハンドラに設定したい場合、handler=main.handlerと設定する)"
}
